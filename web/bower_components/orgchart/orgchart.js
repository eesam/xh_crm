...
var	lineColor = "#000000",		// Color of the connection lines (global for all lines)
	...
	vSpace = 50,			// Vertical space in between the boxes (global for all boxes)
	hShift = 15,			// The number of pixels vertical siblings are shifted (global for all boxes)
	boxLineColor = "#FFFFFF",	// Default box line color
	boxFillColor = "#f1f1f1",	// Default box fill color
	textColor = "#000000",		// Default box text color
	textFont = "Microsoft Yahei",		// Default font
	textSize = 18,			// Default text size (pixels, not points)
	...
	lastMousemoveIndex = -1; // 上次鼠标进入的节点Index
...
var	setNodeAt,	// 设置节点
	drawChartPriv,
	orgChartMouseMove,	// 鼠标进入事件
	orgChartClick,		// 鼠标点击事件
...
orgChart.prototype.redrawChart = function (id)
{
	...
	// ctx.scale(0.5, 0.5);
	drawChartPriv(id, false, '', true);
}

orgChart.prototype.clear = function ()
{
	nodes = [];
};
...
// Internal functions:
//////////////////////
setNodeAt = function (index, linecolor, fillcolor, textcolor)
{
	if (index < 0) return;
	if (! linecolor) { linecolor = boxLineColor; }
	if (! fillcolor) { fillcolor = boxFillColor; }
	...
	nodes[index].linecolor = linecolor;
	nodes[index].fillcolor = fillcolor;
	nodes[index].textcolor = textcolor;
};
...
orgChartMouseMove = function(event)
{
	...
	// 处理滚动条偏移
	var elementx = document.getElementById("div-canvas");
	if (elementx && elementx.scrollLeft) {
		x += elementx.scrollLeft;
	}
	var elementy = document.getElementById("page-wrapper");
	if (elementy && elementy.scrollTop) {
		y += elementy.scrollTop;
	}
	...
	if (i >= 0) {
		setNodeAt(lastMousemoveIndex, '', '', '');
		lastMousemoveIndex = i;
		var linecolor = "#0092ff",
			fillcolor = "#0092ff",
			textcolor = "#FFFFFF";
		setNodeAt(i, linecolor, fillcolor, textcolor);
	} else {
		setNodeAt(lastMousemoveIndex, '', '', '');
	}
	drawChartPriv('canvas1', false, '', true);
};
...
drawNode = function(ctx, i)
{
	...
	gradient.addColorStop(0,  bfcolor);
