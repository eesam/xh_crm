...
// 视图控制器
.controller('inboundCtrl', ['$scope', '$modal', '$log', 'inboundService', function($scope, $modal, $log, inboundService) {
	var ctrl = this;
	...
	ctrl.initList = function initList(tableState) {
		...
		ctrl.isLoading = true;
		inboundService.getList(ctrl.start, ctrl.number).then(function (result) {
			if (result.status == 200) {
				if (result.data.Code === 0) {
					var jsonObj = angular.fromJson(result.data.Msg);
					ctrl.displayed = jsonObj.inboundInfos;
	...
	ctrl.openEdit = function(size, selectrow) {
		var modalInstance = $modal.open({
			templateUrl : 'back/inbound/inboundEdit.html',
			controller : 'inboundModal',
			size : size,
			backdrop : 'static',
			resolve : {
	...
	ctrl.openDelete = function(size, selectrow) {
		var modalInstance = $modal.open({
			templateUrl : 'back/inbound/inboundDelete.html',
			controller : 'inboundModal',
			size : size,
			backdrop : 'static',
			resolve : {
...
// 模态框控制器
.controller('inboundModal', ['$scope', '$modalInstance', 'inboundService', 'selectrow', 'modaltype', function($scope, $modalInstance, inboundService, selectrow, modaltype) {
	if (modaltype == 1) {
		inboundService.getDesignList().then(function (result) {
			if (result.status == 200) {
				if (result.data.Code === 0) {
					var jsonObj = angular.fromJson(result.data.Msg);
		...
		inboundService.getColorList().then(function (result) {
			if (result.status == 200) {
				if (result.data.Code === 0) {
					var jsonObj = angular.fromJson(result.data.Msg);
		...
		inboundService.getSupplierList().then(function (result) {
			if (result.status == 200) {
				if (result.data.Code === 0) {
					var jsonObj = angular.fromJson(result.data.Msg);
	...
	$scope.onAdd = function() {
		...
		inboundService.add(designId, colorId, supplierId, quantity, price, time, note).then(function (result) {
			if (result.status == 200) {
				if (result.data.Code === 0) {
					toastr.error("新增成功！！！");
	...
	$scope.onEdit = function() {
		var id = $scope.item.id,
			...
			time = $scope.item.time,
			note = $scope.item.note;
		inboundService.edit(id, designId, colorId, supplierId, quantity, price, time, note).then(function (result) {
			if (result.status == 200) {
				if (result.data.Code === 0) {
					toastr.error("编辑成功！！！");
					$modalInstance.close($scope.selected);
	...
	$scope.onDelete = function() {
		inboundService.remove(selectrow.id).then(function (result) {
			if (result.status == 200) {
				if (result.data.Code === 0) {
					toastr.error("删除成功！！！");
					$modalInstance.close($scope.selected);
...
// 后台交互服务
.factory('inboundService', ['$q', '$filter', '$timeout', '$http', '$cookies', function ($q, $filter, $timeout, $http, $cookies) {
